...
(function () {
  const DICT = {
    'zh-TW': {
      ...
      'onb.next': '下一步',
      'onb.start': '開始使用',
      'onb.s1.title': '嗨，歡迎來到 MD.Piece',
      'onb.s1.desc': '把每天的小碎片（症狀、用藥、情緒）拼起來，看診時可以直接給醫師看。八步帶你看完最常用的功能。',
      'onb.s2.title': '首頁：今日拼圖 + 待辦',
      'onb.s2.desc': '最上面的橘色卡片會自動數你今天打了幾個碎片（目標 6 個）。底下「今日待辦」會列出系統提醒（如服藥、回診倒數）+ 你自己加的事，按 ✓ 完成、🗑 刪除。',
      'onb.s3.title': '症狀紀錄：點身體圖 + AI 分析',
      'onb.s3.desc': '滑鼠或手指在小人身上跟著跑紅點，點下去鎖定位置，按「AI 分析」會推估該部位的可能狀況、建議科別、緊急度。下方還有 6 大症狀分類可以詳細打卡。',
      'onb.s4.title': '藥物紀錄：拍藥單 + 時段打卡',
      'onb.s4.desc': '拿到藥袋直接拍照，OCR 自動讀名稱/劑量；藥會自動分早/午/晚/睡前時段。點卡片就打卡，✗ 跳過、★ 紀錄療效、? 查藥物百科。上方 hero 顯示今日進度（N / M 完成）。',
      'onb.s5.title': '情緒電力：環形電量打卡',
      'onb.s5.desc': '頂端環形顯示你最近一筆電量。下方電池滑桿可選今天 1～5 格（年長版直接點 emoji），加 200 字內備註。電量日曆、趨勢線、表格往下都有，連續低電量會自動提醒醫師。',
      'onb.s6.title': '診前報告：回診前 30 秒帶走',
      'onb.s6.desc': '看診前打開：頂端「回診倒數」卡、本週 Timeline（時序合併症狀＋情緒＋服藥）、AI 整理「最該問醫師的三件事」、近 30 天健康摘要。可下載「患者版/醫師版」PDF 或一鍵複製給醫師。',
      'onb.s7.title': '小禾陪聊：整理近況 + 寫文章',
      'onb.s7.desc': '不知道怎麼跟醫師說？跟小禾聊一聊，按「整理近況」自動彙整最近三天的不舒服。「生成文章」會把對話寫成一篇給自己留念。下方建議晶片可直接跳去打卡。',
      'onb.s8.title': '不順手隨時可以調',
      'onb.s8.desc': '右上角可切換語言（中/英）、深淺色、放大字體（年長版）。系統設定裡能重新看引導、調通知、匯出資料。任何頁面上方都有「如何使用？」摺疊說明。',

      // 各頁「如何使用」面板
      'howto.toggle.show': '如何使用這頁？',
      'howto.toggle.hide': '收起說明',
      'howto.meds.title': '藥物紀錄 · 怎麼用',
      'howto.meds.s1': '拍藥袋／處方：按上方「拍攝藥單」或「上傳照片」，MD.Piece 會自動辨識藥名、劑量、頻率',
      'howto.meds.s2': '辨識完按「儲存」加入清單；不準的地方可手動微調再存，或直接用「手動新增」自己打',
      'howto.meds.s3': '清單會自動分早晨／中午／晚上／睡前／其他 5 個時段。「其他」是 PRN（需要時）或每 X 小時的藥',
      'howto.meds.s4': '時間到了按藥卡 → 打卡完成；✗＝今天沒吃／跳過、★＝紀錄療效與副作用、?＝查藥物百科',
      'howto.meds.s5': '最上方橘色 hero「今日進度」會自動算 N / M 完成；下方有 30 天服藥趨勢線、改善曲線',
      'howto.meds.s6': '想要報告？右下「生成藥物報告」會把指定區間整理成可印出文件',
      'howto.meds.warn': '所有服藥決策請以醫師處方／藥師為主；本工具僅供記錄與提醒',
      'howto.symptoms.title': '症狀紀錄 · 怎麼用',
      'howto.symptoms.s1': '在身體圖上滑動，紅點會跟著游標跑；點下去鎖定位置',
      'howto.symptoms.s2': '按「✨ 用 AI 分析這個位置可能的狀況」，AI 會推測可能病因、建議科別、緊急度',
      'howto.symptoms.s3': '想留長期紀錄就往下選症狀分類（頭痛、腸胃…），按強度（1-10）+ 頻率 + 備註',
      'howto.symptoms.s4': '紀錄完會出現在「最近紀錄」清單，也會自動進到診前報告',
      'howto.symptoms.warn': 'AI 分析僅供與醫師討論，不取代專業診斷',
      'howto.emotions.title': '情緒電力 · 怎麼用',
      'howto.emotions.s1': '頂端環形電量顯示你最近一筆的電量百分比 + 等級',
      'howto.emotions.s2': '中央「今天剩多少電？」滑動電池選 1～5 格（年長版直接點 emoji），加 200 字內備註',
      'howto.emotions.s3': '按「送出今日打卡」存起來，環形會即時更新',
      'howto.emotions.s4': '下方有電量日曆（月）、趨勢折線圖（7／30／90 天）、表格',
      'howto.emotions.s5': '連續多天低電量系統會在診前報告中標註，方便跟醫師討論',
      'howto.previsit.title': '診前報告 · 怎麼用',
      'howto.previsit.s1': '上方「回診倒數」hero 顯示下次回診日期與天數（沒設過就點去設定）',
      'howto.previsit.s2': 'Timeline 自動合併最近 14 天的症狀、情緒、服藥紀錄，依時序列出',
      'howto.previsit.s3': '「最該問醫師的三件事」是 AI 從你的紀錄整理出的提問清單',
      'howto.previsit.s4': '「30 天健康摘要」可下載：患者版（白話）/ 醫師版（臨床），或一鍵複製給醫師',
      'howto.previsit.s5': '出門前打開這頁，30 秒就知道這次該說什麼',
      'howto.previsit.warn': '報告為患者自填紀錄整理，未經臨床檢查，請以醫師判斷為準',
      ...
      'meds.list.empty.desc': '拍下藥袋／藥單照片，MD.Piece 會自動辨識藥名、劑量與用法。',
    ...
    'en': {
      ...
      'howto.emotions.s2': 'Center: slide the battery to pick 1–5 bars (senior mode: tap emoji directly), add a short note (≤200 chars).',
