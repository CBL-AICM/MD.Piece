...
var _dp = { patients: [], filtered: [], selectedId: null };

function enterDoctorPortal() {
  var portal = document.getElementById('doctor-portal');
  var appWrap = document.getElementById('app-wrapper');
  if (appWrap) appWrap.classList.remove('show');
  if (!portal) return;
  portal.hidden = false;
  // 同步 theme
  var theme = appWrap ? (appWrap.dataset.theme || 'dark') : 'dark';
  portal.dataset.theme = theme;
  var themeIcon = document.getElementById('dp-theme-icon');
  if (themeIcon) themeIcon.textContent = theme === 'dark' ? '☾' : '☀';
  // 醫師暱稱
  var u = getCurrentUser() || {};
  var nameEl = document.getElementById('dp-user-name');
  if (nameEl) nameEl.textContent = (u.nickname || u.username || '醫師') + ' 醫師';
  loadDoctorPatients();
  if (typeof lucide !== 'undefined') setTimeout(function() { lucide.createIcons(); }, 30);
}

async function loadDoctorPatients() {
  var listEl = document.getElementById('dp-patient-list');
  if (listEl) listEl.innerHTML = '<p class="dp-empty">載入中…</p>';
  try {
    var res = await fetch(API + '/auth/users');
    if (!res.ok) throw new Error('HTTP ' + res.status);
    var data = await res.json();
    var users = (data && data.users) || [];
    _dp.patients = users.filter(function(x) { return x.role === 'patient'; });
    _dp.filtered = _dp.patients.slice();
    renderDoctorPatientList();
  } catch (e) {
    if (listEl) listEl.innerHTML = '<p class="dp-empty dp-error">載入失敗：' + escapeHtml(e.message || '未知錯誤') + '</p>';
  }
}

function filterDoctorPatients() {
  var q = (document.getElementById('dp-search').value || '').trim().toLowerCase();
  if (!q) {
    _dp.filtered = _dp.patients.slice();
  } else {
    _dp.filtered = _dp.patients.filter(function(p) {
      var nick = (p.nickname || '').toLowerCase();
      var user = (p.username || '').toLowerCase();
      return nick.indexOf(q) >= 0 || user.indexOf(q) >= 0;
    });
  }
  renderDoctorPatientList();
}
...
function renderDoctorPatientList() {
  var listEl = document.getElementById('dp-patient-list');
  if (!listEl) return;
  if (!_dp.filtered.length) {
    listEl.innerHTML = '<p class="dp-empty">沒有符合的病患</p>';
    return;
  }
  listEl.innerHTML = _dp.filtered.map(function(p) {
    var avatar = p.avatar_url
      ? '<img src="' + escapeHtml(p.avatar_url) + '" alt="" />'
      : '<span class="dp-avatar-fallback" style="background:' + (p.avatar_color || '#5B9FE8') + '22;color:' + (p.avatar_color || '#5B9FE8') + '">' + escapeHtml((p.nickname || '?').slice(0,1)) + '</span>';
    var active = _dp.selectedId === p.id ? ' active' : '';
    return '<button class="dp-patient-item' + active + '" onclick="selectDoctorPatient(\'' + p.id + '\')">' +
      '<span class="dp-patient-avatar">' + avatar + '</span>' +
      '<span class="dp-patient-meta">' +
        '<span class="dp-patient-name">' + escapeHtml(p.nickname || '未命名') + '</span>' +
        '<span class="dp-patient-user">@' + escapeHtml(p.username || '') + '</span>' +
      '</span>' +
    '</button>';
  }).join('');
}
...
function selectDoctorPatient(id) {
  _dp.selectedId = id;
  renderDoctorPatientList();
  var patient = _dp.patients.find(function(x) { return x.id === id; });
  if (!patient) return;
  renderDoctorPatientDetail(patient);
}

function renderDoctorPatientDetail(p) {
  var detail = document.getElementById('dp-detail');
  if (!detail) return;
  var avatar = p.avatar_url
    ? '<img src="' + escapeHtml(p.avatar_url) + '" alt="" />'
    : '<span class="dp-avatar-fallback dp-avatar-lg" style="background:' + (p.avatar_color || '#5B9FE8') + '22;color:' + (p.avatar_color || '#5B9FE8') + '">' + escapeHtml((p.nickname || '?').slice(0,1)) + '</span>';
  detail.innerHTML =
    '<header class="dp-d-head">' +
      '<span class="dp-d-avatar">' + avatar + '</span>' +
      '<div class="dp-d-meta">' +
        '<h2>' + escapeHtml(p.nickname || '未命名') + '</h2>' +
        '<p>@' + escapeHtml(p.username || '') + ' · 病患 ID <code>' + escapeHtml((p.id || '').slice(0, 8)) + '…</code></p>' +
      '</div>' +
    '</header>' +
    '<div class="dp-d-grid">' +
      '<section class="dp-card" id="dp-card-emotion"><h3><i data-lucide="heart-pulse" style="width:14px;height:14px"></i> 情緒趨勢（30 天）</h3><div class="dp-card-body">載入中…</div></section>' +
      '<section class="dp-card" id="dp-card-meds"><h3><i data-lucide="pill" style="width:14px;height:14px"></i> 用藥與服藥率</h3><div class="dp-card-body">載入中…</div></section>' +
      '<section class="dp-card dp-card-wide" id="dp-card-symptoms"><h3><i data-lucide="scan-search" style="width:14px;height:14px"></i> 症狀分析記錄</h3><div class="dp-card-body">載入中…</div></section>' +
      '<section class="dp-card dp-card-wide" id="dp-card-notes"><h3><i data-lucide="notebook-pen" style="width:14px;height:14px"></i> 醫師備註</h3>' +
        '<div class="dp-note-form">' +
          '<textarea id="dp-note-input" placeholder="寫下這次的觀察、處置建議、下次追蹤重點…" rows="3"></textarea>' +
          '<div class="dp-note-row">' +
            '<input id="dp-note-focus" type="text" placeholder="下次追蹤重點（可選）" />' +
            '<button class="dp-note-submit" onclick="submitDoctorNote(\'' + p.id + '\')"><i data-lucide="send" style="width:14px;height:14px"></i> 新增備註</button>' +
          '</div>' +
        '</div>' +
        '<div class="dp-card-body" id="dp-notes-list">載入中…</div>' +
      '</section>' +
    '</div>';
  if (typeof lucide !== 'undefined') lucide.createIcons();
  loadDoctorPatientData(p.id);
}

async function loadDoctorPatientData(pid) {
  var pidQ = encodeURIComponent(pid);
  // 平行抓取
  var emotionP = fetch(API + '/emotions/trend?patient_id=' + pidQ + '&days=30').then(function(r) { return r.ok ? r.json() : null; }).catch(function() { return null; });
  var medsP = fetch(API + '/medications/?patient_id=' + pidQ).then(function(r) { return r.ok ? r.json() : null; }).catch(function() { return null; });
  var statsP = fetch(API + '/medications/stats?patient_id=' + pidQ + '&days=30').then(function(r) { return r.ok ? r.json() : null; }).catch(function() { return null; });
  var sympP = fetch(API + '/symptoms/history/' + pidQ).then(function(r) { return r.ok ? r.json() : null; }).catch(function() { return null; });
  var notesP = fetch(API + '/doctor-notes/?patient_id=' + pidQ).then(function(r) { return r.ok ? r.json() : null; }).catch(function() { return null; });

  var results = await Promise.all([emotionP, medsP, statsP, sympP, notesP]);
  renderDpEmotion(results[0]);
  renderDpMeds(results[1], results[2]);
  renderDpSymptoms(results[3]);
  renderDpNotes(results[4]);
  if (typeof lucide !== 'undefined') lucide.createIcons();
}

function renderDpEmotion(data) {
  var box = document.querySelector('#dp-card-emotion .dp-card-body');
  if (!box) return;
  if (!data || !data.trend || !data.trend.length) {
    box.innerHTML = '<p class="dp-empty">最近 30 天無情緒紀錄</p>';
    return;
  }
  var avg = data.average_score != null ? data.average_score : '—';
  var max = 5;
  var bars = data.trend.slice(-30).map(function(t) {
    var pct = t.score ? (t.score / max * 100) : 0;
    var color = t.score >= 4 ? 'var(--accent)' : (t.score >= 3 ? 'var(--purple)' : 'var(--rose)');
    return '<span class="dp-bar" title="' + escapeHtml(t.date) + '：' + t.score + ' 分" style="height:' + pct + '%; background:' + color + '"></span>';
  }).join('');
  box.innerHTML =
    '<div class="dp-stat-row">' +
      '<div class="dp-stat"><span class="dp-stat-num">' + avg + '</span><span class="dp-stat-lbl">平均分數</span></div>' +
      '<div class="dp-stat"><span class="dp-stat-num">' + data.total_records + '</span><span class="dp-stat-lbl">紀錄筆數</span></div>' +
    '</div>' +
    '<div class="dp-bars">' + bars + '</div>';
}

function renderDpMeds(medsData, statsData) {
  var box = document.querySelector('#dp-card-meds .dp-card-body');
  if (!box) return;
  var meds = (medsData && medsData.medications) || [];
  var active = meds.filter(function(m) { return m.active !== 0; });
  var stats = statsData || {};
  var rate = stats.adherence_rate != null ? stats.adherence_rate : null;
  var rateBlock = rate != null
    ? '<div class="dp-stat"><span class="dp-stat-num">' + rate + '%</span><span class="dp-stat-lbl">服藥率（30天）</span></div>'
    : '<div class="dp-stat"><span class="dp-stat-num">—</span><span class="dp-stat-lbl">服藥率</span></div>';
  var medList = active.length
    ? '<ul class="dp-med-list">' + active.slice(0, 8).map(function(m) {
        return '<li><strong>' + escapeHtml(m.name || '未命名') + '</strong><span>' + escapeHtml(m.dosage || '') + ' · ' + escapeHtml(m.frequency || '') + '</span></li>';
      }).join('') + '</ul>'
    : '<p class="dp-empty">無使用中的藥物</p>';
  box.innerHTML =
    '<div class="dp-stat-row">' +
      '<div class="dp-stat"><span class="dp-stat-num">' + active.length + '</span><span class="dp-stat-lbl">使用中藥物</span></div>' +
      rateBlock +
    '</div>' +
    medList;
}

function renderDpSymptoms(data) {
  var box = document.querySelector('#dp-card-symptoms .dp-card-body');
  if (!box) return;
  var hist = (data && data.history) || [];
  if (!hist.length) {
    box.innerHTML = '<p class="dp-empty">尚無症狀分析紀錄</p>';
    return;
  }
  box.innerHTML = '<ul class="dp-symp-list">' + hist.slice(0, 10).map(function(s) {
    var symptoms = Array.isArray(s.symptoms) ? s.symptoms.join('、') : (s.symptoms || '');
    var ai = s.ai_response || {};
    var summary = ai.summary || ai.assessment || '';
    var when = (s.created_at || '').slice(0, 16).replace('T', ' ');
    return '<li>' +
      '<div class="dp-symp-when">' + escapeHtml(when) + '</div>' +
      '<div class="dp-symp-tags">' + escapeHtml(symptoms) + '</div>' +
      (summary ? '<div class="dp-symp-summary">' + escapeHtml(summary) + '</div>' : '') +
    '</li>';
  }).join('') + '</ul>';
}

function renderDpNotes(data) {
  var box = document.getElementById('dp-notes-list');
  if (!box) return;
  var notes = (data && data.notes) || [];
  if (!notes.length) {
    box.innerHTML = '<p class="dp-empty">尚無備註，使用上方表單新增</p>';
    return;
  }
  box.innerHTML = '<ul class="dp-note-list">' + notes.map(function(n) {
    var when = (n.created_at || '').slice(0, 16).replace('T', ' ');
    return '<li>' +
      '<div class="dp-note-when">' + escapeHtml(when) + '</div>' +
      '<div class="dp-note-content">' + escapeHtml(n.content || '') + '</div>' +
      (n.next_focus ? '<div class="dp-note-focus"><i data-lucide="target" style="width:12px;height:12px"></i> ' + escapeHtml(n.next_focus) + '</div>' : '') +
    '</li>';
  }).join('') + '</ul>';
  if (typeof lucide !== 'undefined') lucide.createIcons();
}

async function submitDoctorNote(patient_id) {
  var input = document.getElementById('dp-note-input');
  var focus = document.getElementById('dp-note-focus');
  var content = (input.value || '').trim();
  if (!content) {
    showToast('請先輸入備註內容', 'warning');
    return;
  }
  var doctor = getCurrentUser() || {};
  try {
    var res = await fetch(API + '/doctor-notes/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        content: content,
        next_focus: (focus.value || '').trim() || null,
      })
    });
    if (!res.ok) {
    ...
    // 重新載入備註列表
    var nres = await fetch(API + '/doctor-notes/?patient_id=' + encodeURIComponent(patient_id));
    if (nres.ok) renderDpNotes(await nres.json());
  } catch (e) {
...
(function bootstrap() {
  var u = getCurrentUser();
  if (u && u.role === 'doctor') return; // doctor portal 由 enterApp / finishAuth 啟動
  showPage("home");
})();
