...
function showPage(page) {
  ...
  const pages = {
    home, symptoms, analysis, doctors, patients, records, medications, education,
...
function home() {
  ...
  return `
    <div class="home-page">
      ...
      <div class="home-grid">
        ${homeCard('symptoms','scan-search','症狀紀錄','留下身體每天的訊號','blue')}
        ${homeCard('analysis','sparkles','症狀分析','AI 推測可能原因＋第一步怎麼做','rose')}
...
// ─── 症狀紀錄 ──────────────────────────────────────────────
// 這裡是「紀錄」：把症狀、強度、頻率留檔。
// 想知道「可能原因」與「先怎麼處理」請到 analysis() 頁面（症狀分析）。
    ...
    <div class="sym-page">
      ...
      <section class="term-section">
        ...
        <div class="ts-body">
          ...
          <div class="lf-actions" style="margin-top:8px;flex-wrap:wrap;gap:8px">
            <button class="sym-link-btn" onclick="openVisitDatePrompt()">
              <i data-lucide="calendar-cog"></i><span>設定回診日期</span>
            </button>
            <button class="sym-link-btn" onclick="analyzeRecordedSymptoms()" title="拿這些紀錄去做 AI 分析">
              <i data-lucide="sparkles"></i><span>分析這些症狀 →</span>
            </button>
          </div>
        </div>
      </section>
...
// ─── 症狀分析（與症狀紀錄分開：紀錄 = 留檔；分析 = 找原因＋第一步怎麼做）──

function analysis() {
  const stats = getPeriodStats();
  const recordedNames = Object.keys(stats.byCategory).map(id => {
    const c = SYMPTOM_CATEGORIES.find(x => x.id === id);
    return c ? c.zh : id;
  });
  const prefilled = recordedNames.join('、');
  ...
  return `
    <div class="sym-page">
      ...
      <section class="term-section">
        <header class="ts-head">
          <span class="ts-prompt">$ ./analyze-symptoms</span>
          <span class="ts-tag">cause + first_aid</span>
        </header>
        <div class="ts-body">
          <p class="sym-instruct">
            告訴 AI 你正在經歷的症狀，它會幫你想<strong>可能的原因</strong>，
            並建議<strong>第一步可以先怎麼做</strong>。
          </p>
          <p class="sym-instruct" style="opacity:.7">
            ⚠️ 分析僅供參考，不取代醫師診斷。若情況惡化請就醫。
          </p>

          <label class="lf-label" style="margin-top:8px">輸入症狀（多個請用逗號分隔）</label>
          <textarea id="analysis-input" rows="3"
            placeholder="例如：頭痛, 發燒, 喉嚨痛">${escapeHtml(prefilled)}</textarea>

          ${recordedNames.length > 0 ? `
          <p class="sym-instruct" style="margin-top:6px;font-size:.9em;opacity:.75">
            ↑ 已自動帶入此期間「症狀紀錄」中出現的 ${recordedNames.length} 項，可自行調整。
          </p>` : `
          <p class="sym-instruct" style="margin-top:6px;font-size:.9em;opacity:.75">
            // 還沒有任何紀錄。可以直接輸入現在的症狀，或先去
            <a href="javascript:void(0)" onclick="navigateTo('symptoms', null)">症狀紀錄</a> 留下資料。
          </p>`}

          <div class="lf-actions" style="margin-top:12px">
            <button class="primary-btn" onclick="runAnalysis()" type="button">
              <i data-lucide="scan-search"></i><span>開始分析</span>
            </button>
            <button class="secondary-btn" onclick="navigateTo('symptoms', null)" type="button">
              ← 回症狀紀錄
            </button>
          </div>
        </div>
      ...
      <section class="term-section" id="analysis-output" style="display:none">
        <header class="ts-head">
          <span class="ts-prompt">$ ai_response</span>
          <span class="ts-tag" id="analysis-output-tag">analyzing...</span>
        </header>
        <div class="ts-body" id="analysis-result"></div>
      </section>
...
async function runAnalysis() {
  ...
  if (!raw) {
    showToast('請先輸入至少一個症狀', 'warning');
    return;
  }
  const symptoms = raw.split(/[,，、\n]/).map(s => s.trim()).filter(Boolean);

  const out = document.getElementById('analysis-output');
  const tag = document.getElementById('analysis-output-tag');
  const el  = document.getElementById('analysis-result');
  out.style.display = 'block';
  tag.textContent = 'analyzing...';
  el.innerHTML = '<p class="sym-instruct">// AI 分析中，請稍候…</p>';
  ...
  try {
    ...
    const urgencyMap = {
      emergency: { label: '緊急', cls: 'urgency-emergency', hint: '建議立即就醫或撥打 119' },
      high:      { label: '高',   cls: 'urgency-high',      hint: '盡快安排就醫' },
      medium:    { label: '中',   cls: 'urgency-medium',    hint: '近日內就醫評估' },
      low:       { label: '低',   cls: 'urgency-low',       hint: '可先觀察並自我照顧' },
    };
    const urg = urgencyMap[data.urgency] || urgencyMap.low;

    const conditions = (data.conditions || []).length === 0
      ? '<li>暫無明確可能原因，建議由醫師評估</li>'
      : data.conditions.map(c =>
          `<li><strong>${escapeHtml(c.name || '')}</strong> — 可能性：${escapeHtml(c.likelihood || 'N/A')}</li>`
        ).join('');

    tag.textContent = symptoms.length + ' symptom' + (symptoms.length === 1 ? '' : 's');
    el.innerHTML = `
      <div class="ai-result-card">
        <div class="urgency-badge ${urg.cls}">緊急程度：${urg.label} · ${urg.hint}</div>
        <h4>可能原因</h4>
        <ul>${conditions}</ul>
        <h4>第一步可以先怎麼做</h4>
        <p>${escapeHtml(data.advice || '建議就醫，請諮詢醫師。')}</p>
        <h4>建議就診科別</h4>
        <p>${escapeHtml(data.recommended_department || '家醫科')}</p>
        <div class="disclaimer">${escapeHtml(data.disclaimer || '此分析僅供參考，不構成醫療診斷。如有不適請立即就醫。')}</div>
      </div>`;
    if (typeof lucide !== 'undefined') lucide.createIcons();
  } catch (e) {
    tag.textContent = 'error';
    el.innerHTML = '<div class="advice-box">分析失敗，請確認後端是否啟動，或稍後再試。</div>';
  }
}

// 從「症狀紀錄」一鍵帶資料到「症狀分析」
function analyzeRecordedSymptoms() {
  navigateTo('analysis', null);
}
