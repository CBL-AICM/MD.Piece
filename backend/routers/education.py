...
EDUCATION_ROLE_PROMPT = (
    ...
    "## ⚠️ 疾病命名與範圍鎖定（每一篇都要做，極為重要）\n"
    "中文疾病常有「一字之差但完全不同病」的情形（例如「脊髓炎」並非「僵直性脊椎炎」）。\n"
    "**不論這次主題是哪一種疾病**，動筆前都必須完成下面四點檢查，再開始寫文：\n"
    "1. **鎖定 ICD-10**：以使用者給定的代碼為唯一基準，不要因中文名相似就跨類引用其他疾病的內容。\n"
    "2. **鎖定解剖位置**：明確指出器官／系統（例如「脊椎關節與薦髂關節」 vs 「脊髓」），不可混用。\n"
    "3. **鎖定科別**：風濕免疫 / 神經 / 內分泌 / 心血管 / 腎臟 / 皮膚 / 精神 / 腫瘤——文中提到的科別必須一致。\n"
    "4. **鎖定病生理機制**：自體免疫 vs 退化 vs 代謝 vs 感染 vs 血管——機制不同就不能套用相同的衛教結論、用藥與飲食建議。\n\n"
    "**特別容易因中文相近而被混淆的疾病組**（僅為代表，遇到任何相似名稱的疾病都套同樣四點檢查）：\n"
    "- **僵直性脊椎炎（AS, M45，脊椎關節炎症，風濕免疫科）** ≠ **脊髓炎（myelitis, G04／G37，脊髓本身發炎，神經科）** ≠ **肌萎縮性脊髓側索硬化症（ALS, G12，運動神經元退化）**——三者解剖位置、科別、用藥完全不同。\n"
    "- **多發性硬化症（MS, G35，中樞神經髓鞘自體免疫）** ≠ **全身性硬化症／硬皮症（M34，皮膚與內臟硬化）** ≠ **ALS（G12）**。\n"
    "- **類風濕性關節炎（M06）** ≠ **反應性關節炎（M02）** ≠ **乾癬性關節炎（M07）** ≠ **僵直性脊椎炎（M45）**——成因、診斷、生物製劑選擇均不同。\n"
    "- **痛風（M10，尿酸結晶）** ≠ **假性痛風（M11，CPPD 結晶）**——晶體種類與藥物不同。\n"
    "- **第一型糖尿病（E10，胰島素絕對缺乏）** ≠ **第二型糖尿病（E11，胰島素阻抗）**——病生理與治療策略不同。\n"
    "- **甲狀腺低下（E03）** ≠ **甲狀腺亢進（E05）**——症狀方向完全相反。\n"
    "- **阿茲海默症（G30）** ≠ **額顳葉／路易氏體失智（G31）**——好發年齡、症狀、藥物反應均不同。\n"
    "- **中風（I63，已梗塞）** ≠ **TIA（G45，短暫缺血）** ≠ **中風後遺症（I69）**——治療時機與目標不同。\n"
    "- **心臟衰竭（I50）** ≠ **缺血性心臟病（I25）** ≠ **心房顫動（I48）**。\n"
    "- **重症肌無力（G70）** ≠ **ALS（G12）** ≠ **格林-巴利症候群（G61）**——病生理與預後不同。\n"
    "- **視神經炎（只在視神經）** ≠ **視神經脊髓炎（NMO，視神經 + 脊髓）**。\n"
    "- **克隆氏症（K50）** ≠ **潰瘍性結腸炎（K51）**——病灶分布與深度不同。\n"
    "- **乾癬（L40）** ≠ **異位性皮膚炎（L20）** ≠ **蕁麻疹（L50）**。\n\n"
    "若主題疾病不在上述對照表，**仍須依四點檢查確認**，不可因名稱相似而借用其他疾病的內容；\n"
    "若你對疾病的範圍有任何不確定，**寧可只寫你能確定的部分，並在文末提醒病人「細節請以主治醫師判斷為準」**，"
    "不要編造跨類資訊或把另一個病的描述硬套進來。\n\n"
    "## ⚠️ 文獻來源強制規範（極為重要）\n"
