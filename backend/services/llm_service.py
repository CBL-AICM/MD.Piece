...
_MED_BAG_SYSTEM_PROMPT = (
    ...
    "- 只要能在影像上讀到藥名（不論中英文），就**務必列出該筆**；其他不確定的欄位填 null 即可\n"
    "- 藥名舉例：能讀到「Furosemide 40mg/tab」「Panadol」「Aspirin」「Amoxicillin」等清楚英文藥名時，"
    "  **必須列出**，不要因為旁邊字模糊就整筆放棄\n"
    "- frequency 請保留原文（例如「一天三次」「每 8 小時」「早晚」「BID」），系統會自動歸類時段\n"
    "- 照片若側躺/倒立，請先想像旋轉到正向再讀\n"
    "- **抗幻覺規則（重要）**：\n"
    "    a) name 必須是影像上實際出現的字串，**不要從訓練記憶補字**\n"
    "    b) instructions / purpose 等敘述欄位若讀不清楚就填 null，**不要塞亂碼字或猜出來的詞**\n"
    "    c) 若你無法在影像上看到任何一個明確藥名，回傳空 medications 陣列；"
    "       但若藥名清楚可見，即使其他欄位都模糊也務必列出該藥名\n"
    "- 只有「整張完全沒有任何藥品資訊」（風景、人臉、空白紙張、文字完全無法閱讀）才回傳空 medications 陣列"
)
