...
_MED_BAG_SYSTEM_PROMPT = (
    ...
    "  - frequency   服用頻率（原文照抄；例：一天三次、每 8 小時一次、需要時服用、早晚）\n"
    ...
    "- frequency 請保留原文（例如「一天三次」「每 8 小時」「早晚」），系統會自動歸類時段\n"
    "- 寧可缺欄位，不要填入錯誤資訊\n"
    "- 即便藥袋拍得不完整、模糊、被光線遮住一部分，也請盡量從可見字元辨識，"
    "  不要因為照片不完美就回傳空陣列；至少把可辨識的藥名先列出來"
)

_MED_BAG_USER_PROMPT = "請辨識這張藥袋上的所有藥物資訊，逐包分開列出。"
