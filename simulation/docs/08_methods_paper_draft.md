...
no-dropout upper bound reached ΔCRS +0.235, quantifying the engagement penalty. A derived
patient-reported-outcome layer mirrored the same crossover: adoption 66%, month-3 active 40%,
mean satisfaction 50/100, whole-population NPS −48, with satisfaction correlated to record
improvement (r = 0.75).
...
### 3.6 Usage, sentiment, and patient-reported outcomes
Extending the analysis to the usage dimension, a derived patient-reported-outcome (PRO) layer —
structured on the MAUQ usability subscales (ease-of-use, interface/satisfaction, usefulness; Mustafa
2021, [DOI](https://doi.org/10.2196/24457)) and the sentiment themes of Kruse 2015 (top positive =
patient–provider communication; top negatives = usability/security, worst for the tech-unfamiliar;
[DOI](https://doi.org/10.2196/jmir.3703)) — produced a crossover that **mirrors** the fidelity result.
Adoption was 66%, month-3 active 40%; mean satisfaction (MAUQ-style 0–100) was 50 with a
whole-population NPS of −48, and satisfaction correlated with how much the record improved (r = 0.75).
By persona, satisfaction ran from Perfect-Logger 75 and Caregiver-Managed 71 down to Low-Engagement 36
and Technology-Avoidant 30. Thus **usage (adoption/retention), sentiment (satisfaction/NPS), and record
fidelity (ΔCRS) trace the same crossover and reinforce one another**. This PRO layer is a derived,
literature-structured estimate (expert judgment, validation-required); illustrative patient "voices"
are simulated, not real. (Generated by `make_usage_research.py`; full write-up in
`docs/10_usage_sentiment_research.md`.)
...
than universal rollout, and for caregiver-proxy logging as a first-class feature. The usage and
sentiment dimensions (§3.6) converge on the same segments, reinforcing the targeting conclusion.
...
via `python -m simulation.run_study`, `run_sensitivity`, `run_sobol`, `make_figures`,
`make_data_figures`, and `make_usage_research`. Generated datasets are
